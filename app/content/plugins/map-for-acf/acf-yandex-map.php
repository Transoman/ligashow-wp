...
if ( ! function_exists( 'the_yandex_map' ) ) {
	 ...
	 * @param null $data
	 */
	function the_yandex_map( $selector, $post_id = false, $data = null, $map_icon = null ) {
		...
		$field_height = $field['height'] ? $field['height'] : 200;
		$height_map   = apply_filters( 'acf-yandex-map/height', $field_height, $selector, $post_id, $value );
		$map_icon = $map_icon ? $map_icon : 'default';

		echo sprintf( '<div class="yandex-map" id="%s" data-icon="%s"></div>', $map_id, $map_icon );
	}
